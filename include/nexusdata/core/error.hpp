...
/// Prefix an exception with the pipeline step, file, or call that failed.
/// The original message stays at the end, so nested calls form a chain.
[[nodiscard]] inline std::string with_context(const std::string& where, const std::exception& error) {
    return where + ": " + error.what();
}
