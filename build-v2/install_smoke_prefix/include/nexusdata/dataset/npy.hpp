...
/// Keeps mmap alive for zero-copy NDArray views. v2.0: `.npy.gz` / `.npy.zst` / ...
/// are decoded into memory once and viewed the same way.
class NpyFile {
public:
...
/// Minimal NPZ reader: stored members (np.savez) and, with NEXUSDATA_WITH_ZLIB,
/// deflated members (np.savez_compressed).
class NpzFile {
public:
